...
def median_filter_color(image_array, kernel_size=3):
    """
    对彩色图像应用中值滤波。
    参数：
...
def main():
    ...
    # 打开输入图像
    image_path = "rayleigh_noise_colored.jpg"
    ...
    filtered_array = median_filter_color(input_array, kernel_size)
    end_time = time.time()
    ...
    # 保存滤波后的图像
    output_path = os.path.join(output_dir, "rayleigh_noise_median_filter_colored.jpg")
