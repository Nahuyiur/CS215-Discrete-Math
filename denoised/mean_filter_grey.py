...
def mean_filter_grey(image_array, kernel_size=3):
    """
    对灰度图像应用均值滤波。
    参数：
...
def main():
    ...
    # 打开输入图像
    image_path = "uniform_noise_grey.jpg"
    input_image = Image.open(image_path).convert('L')  # 转为灰度图
    input_array = np.array(input_image)

    # 应用均值滤波
    kernel_size = 5  # 可调节核大小
    start_time = time.time()
    filtered_array = mean_filter_grey(input_array, kernel_size)
    end_time = time.time()
    ...
    # 保存滤波后的图像
    output_path = os.path.join(output_dir, "uniform_noise_mean_filtered_grey.jpg")
